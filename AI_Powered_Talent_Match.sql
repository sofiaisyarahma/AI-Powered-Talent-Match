...
params AS (
  SELECT
    ...
    ARRAY[{{ selected_talent_ids }}]::TEXT[] AS selected_talent_ids,  -- e.g. ARRAY[1001,1005,1010]
...
tv_union AS (
  -- numeric TVs
  SELECT employee_id, 'gtq' AS tv_name, gtq_score::TEXT AS tv_value,
         'Cognitive Complexity & Problem-Solving' AS tgv_name, 'numeric' AS data_type
  FROM profiles_psych
  UNION ALL
  SELECT employee_id, 'iq', iq_score::TEXT, 'Cognitive Complexity & Problem-Solving', 'numeric' FROM profiles_psych
  UNION ALL
  SELECT employee_id, 'tiki', tiki_score::TEXT, 'Cognitive Complexity & Problem-Solving', 'numeric' FROM profiles_psych
  UNION ALL
  SELECT employee_id, 'pauli', pauli_score::TEXT, 'Motivation & Drive', 'numeric' FROM profiles_psych

  -- PAPI
  UNION ALL
  SELECT employee_id, column_name AS tv_name, column_value::TEXT AS tv_value,
         CASE
  ...
  -- categorical TVs
  UNION ALL
  SELECT employee_id, 'mbti_type', mbti_type::TEXT AS tv_value,
         'Creativity & Innovation Orientation', 'categorical'
  FROM profiles_psych
  UNION ALL
  SELECT employee_id, 'disc_type', disc::TEXT AS tv_value,
         'Leadership & Influence', 'categorical'
  FROM profiles_psych
  ...
  SELECT 
      employee_id,
      CONCAT('strength_', rank_id) AS tv_name,
      theme::TEXT AS tv_value,
      'Motivation & Drive' AS tgv_name,
  ...
  SELECT
	...
	CASE
      ...
      WHEN t.tv_name NOT IN ('papi_k') THEN
        LEAST(((t.tv_value)::NUMERIC / NULLIF(b.benchmark_value, 0)) * 100, 100)

      -- Inverse scoring for PAPI_K (lower = better)
      WHEN t.tv_name = 'papi_k' THEN
        LEAST(((2 * b.benchmark_value - (t.tv_value)::NUMERIC) / NULLIF(b.benchmark_value, 0)) * 100, 100)
    END
      WHEN t.data_type = 'categorical' THEN
        CASE
          -- exact match for DISC and CliftonStrengths
        WHEN t.tv_name LIKE 'strength_%%' 
        AND t.tv_value IN (
            SELECT theme
            FROM strengths
            CROSS JOIN LATERAL (
            ...
            WHERE employee_id = ANY(ARRAY(SELECT unnest(selected_talent_ids) FROM params))
          ) THEN 100
          ELSE 0
        END
...
GROUP BY p.job_vacancy_id, p.role_name, p.role_level, p.role_purpose,
         e.employee_id, e.fullname, f.final_match_rate
